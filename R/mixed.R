...
.gce_mult_obj <- function(param, Y, X, v, nu, N, K, J, M, H) {

  lambda <- param[1:K * (J - 1)]
  lambda <- matrix(lambda, K, (J - 1))
  ...
  for (i in 1:N) {
    for (j in 1:J) {
      for (m in 1:M) {
        phi[i, j, m] <- exp(s[m] * (-sum(lambda[, j] * x[i, j, ]) - rho[i]))
      }
  ...
  for (i in 1:N) {
    for (j in 1:J) {
      for (h in 1:H) {
        w[i, j, h] <- exp(-v[h] * sum(lambda[, j] * x[i, j, ]))
      }
      Psi[i, j] <- sum(w[i, j, h])
    }
  }
